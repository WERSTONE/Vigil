...
def _resolve_weights(name: str, pretrained: Union[bool, str, None]) -> Union[str, None]:
    """解析权重路径.
...
def create_model(name: str, pretrained: Union[bool, str, None] = True, **kwargs
                 ) -> VigilModelBase:
    """从注册表创建模型实例, 自动加载对应权重.
    ...
    if name not in MODEL_REGISTRY:
        ...
        try:
            import importlib
            importlib.import_module(f"models.{name}.model")
        except ImportError:
            pass
    if name not in MODEL_REGISTRY:
        raise KeyError(
